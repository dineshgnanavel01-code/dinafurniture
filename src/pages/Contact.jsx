...
import {ArrowRight,Check,Clock3,Mail,MapPin,MessageCircle,Phone,Send,
    ...
    <main className="overflow-hidden" style={{ fontFamily: 'var(--font-primary)' }}>
          ...
          <div className="grid items-end gap-10 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              <div className="mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-amber-600 dark:text-amber-400">
                ...
                Let us make room for good things
              </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
        ...
        <div className="mt-12 grid overflow-hidden rounded-3xl border border-stone-200 bg-stone-50 dark:border-stone-800 dark:bg-stone-900/60 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="relative flex flex-col justify-between overflow-hidden bg-stone-900 p-8 text-white sm:p-10">
            <div className="absolute -bottom-20 -right-10 h-56 w-56 rounded-full border border-amber-400/30" />
            <div className="relative flex h-full flex-col justify-between">
              <div>
                <span className="mb-6 flex h-11 w-11 items-center justify-center rounded-full bg-amber-500 text-stone-950">
                  <MapPin className="h-5 w-5" />
                </span>
                <h2 className="text-3xl font-bold" style={{ fontFamily: 'var(--font-secondary)' }}>
                  Come say hello.
                </h2>
                <p className="mt-3 max-w-xs text-sm leading-6 text-stone-300">
                  Our Bengaluru studio is a calm corner to experience the collection in person.
                </p>
              </div>
              <div className="mt-10 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-amber-300">
                <Clock3 className="h-4 w-4" /> Open by appointment
              </div>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-10 lg:p-12">
            {submitted ? (
              <div className="flex h-full min-h-360px flex-col items-start justify-center">
                <span className="mb-6 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400">
                ...
                <p className="mt-4 max-w-md leading-7 text-stone-600 dark:text-stone-300">
                  Thank you for reaching out. A member of our studio team will get back to you within one business day.
                </p>
                <button type="button" onClick={() => { setSubmitted(false); setForm(initialForm); }} className="mt-8 text-sm font-semibold text-amber-700 underline decoration-amber-300 underline-offset-4 transition hover:text-amber-500 dark:text-amber-400">
              ...
              <>
                <div className="mb-8 flex items-start justify-between gap-4">
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-amber-600 dark:text-amber-400">Start a conversation</p>
                    <h2 className="text-3xl font-bold text-stone-900 dark:text-white" style={{ fontFamily: 'var(--font-secondary)' }}>
                      How can we help?
                    </h2>
                  </div>
                  <MessageCircle className="hidden h-7 w-7 text-stone-300 sm:block dark:text-stone-700" />
                </div>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid gap-5 sm:grid-cols-2">
                    <label className="text-xs font-semibold text-stone-700 dark:text-stone-300">
                      Your name
                      <input required name="name" value={form.name} onChange={handleChange} placeholder="Ananya Sharma" className="contact-input" />
                    </label>
                    <label className="text-xs font-semibold text-stone-700 dark:text-stone-300">
                      Email address
                      <input required type="email" name="email" value={form.email} onChange={handleChange} placeholder="you@example.com" className="contact-input" />
                    </label>
                  </div>
                  <div className="grid gap-5 sm:grid-cols-2">
                    <label className="text-xs font-semibold text-stone-700 dark:text-stone-300">
                      Phone number <span className="font-normal text-stone-400">(optional)</span>
                      <input name="phone" value={form.phone} onChange={handleChange} placeholder="+91 98765 43210" className="contact-input" />
                    </label>
                    <label className="text-xs font-semibold text-stone-700 dark:text-stone-300">
                      ...
                      <select name="subject" value={form.subject} onChange={handleChange} className="contact-input">
                        ...
                        <option>Trade & collaborations</option>
                      </select>
                    </label>
                  </div>
                  <label className="block text-xs font-semibold text-stone-700 dark:text-stone-300">
                    Your message
                    <textarea required name="message" value={form.message} onChange={handleChange} rows="5" placeholder="Tell us a little about what you are looking for..." className="contact-input resize-none" />
                  </label>
                  <button type="submit" className="group inline-flex items-center gap-3 rounded-xl bg-stone-900 px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-amber-600 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-amber-500 dark:hover:text-white">
                    <Send className="h-4 w-4 transition group-hover:translate-x-1 group-hover:-translate-y-0.5" /> Send message
                  </button>
                </form>
              </>
            )}
